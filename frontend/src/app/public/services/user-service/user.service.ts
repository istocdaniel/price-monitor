...
export class UserService {
  ...
  login(user: UserI): Observable<LoginResponseI> {
    return this.httpClient.post<LoginResponseI>('http://localhost:3000/api/login', user).pipe(
  ...
  register(user: UserI): Observable<UserI> {
    return this.httpClient.post<UserI>('http://localhost:3000/api/register', user).pipe(
